...
6. **Windows 命令行参数编码 (all modules):** NEVER put Chinese / non-ASCII text directly into a shell command line (e.g. `node ima_api.cjs ... '{"query":"高血压"}'`). On Windows the command-line bytes pass through a GBK/ANSI conversion chain and the Chinese will arrive garbled. For any call whose body contains non-ASCII text — especially **search queries** (`search_knowledge` / `search_note`) — ALWAYS write the body JSON to a UTF-8 file and pass `@/absolute/path/to/body.json` instead. See [Windows 命令行参数编码](#windows-命令行参数编码all-modules).
...
**body 参数编码（Windows 环境必须遵守）：**

`ima_api.cjs` 的 body 参数支持三种形式，**任何包含中文/非 ASCII 文本的调用（尤其是检索 query）都必须使用第 2、3 种形式**，禁止把中文直接写在命令行参数里：

1. **内联 JSON（仅 ASCII 内容时可用）**：`node ima_api.cjs "openapi/list_docs" '{"limit":10}' "$OPTS"` — body 中不得出现任何中文。
2. **body 文件（推荐，含中文时必用）**：把完整 body JSON 写入 UTF-8 文件，参数以 `@` 开头指向该文件（自动剥离 UTF-8 BOM，PowerShell 5.1 写入的 BOM 也能正确处理）：
   ```bash
   # 1) 用 write_to_file / Set-Content -Encoding UTF8 写入 UTF-8 的 body 文件
   # 2) 调用时传 @文件绝对路径
   node "$SKILL_DIR/ima_api.cjs" "openapi/wiki/v1/search_knowledge" "@/tmp/search_body.json" "$OPTS"
   ```
3. **stdin（body 从标准输入读取）**：body 参数传 `-`，内容经管道传入。**仅限纯 ASCII 内容** —— 中文经 Windows 管道会被 GBK/ANSI 字节链路破坏（即使设置了 `$OutputEncoding = UTF8` 也不可靠），含中文时**必须**用 body 文件：
   ```bash
   echo '{"limit":10}' | node "$SKILL_DIR/ima_api.cjs" "openapi/list_docs" "-" "$OPTS"
   ```
...
> **此问题影响所有 API 调用（notes、knowledge-base 等），且存在两个方向：**
>
> **方向 A — 发送（请求 Body）：** PowerShell 5.1 下 `Invoke-RestMethod` 会静默将请求 Body 从 UTF-8 转为系统 ANSI 编码（中文 Windows 为 GBK），即使设置了 `Content-Type: charset=utf-8` 也无效。结果是请求看起来发送成功，但服务端收到的内容已经是乱码，且无任何错误提示。
>
> **方向 B — 读取（捕获脚本输出）：** PowerShell 5.1 捕获 `node ima_api.cjs` 的 stdout 时，默认按系统 ANSI/GBK 解码 UTF-8 字节。即使脚本输出正确，捕获到变量/管道里的中文也会变乱码。必须在调用前设置 `[Console]::OutputEncoding = [System.Text.Encoding]::UTF8`。
...
if ($PSVersionTable.PSVersion.Major -le 5) {
    ...
    $useUtf8Bytes = $true
    # CRITICAL: 设置输出解码为 UTF-8，否则捕获 node 脚本 stdout 的中文会乱码
    [Console]::OutputEncoding = [System.Text.Encoding]::UTF8
} else {
    Write-Host "✅ PowerShell 7+，默认 UTF-8，无需额外处理"
    $useUtf8Bytes = $false
    # PS 7+ 已默认 UTF-8，但显式设置可保证一致性
    [Console]::OutputEncoding = [System.Text.Encoding]::UTF8
}
```
...
> **总结：** 在 PowerShell 5.1 环境中，**所有** API 调用（无论 notes 还是 knowledge-base）都必须将 Body 显式转为 UTF-8 字节数组；同时必须设置 `[Console]::OutputEncoding = [System.Text.Encoding]::UTF8` 才能正确读取脚本输出。不检测版本直接发请求 = 中文内容必乱码。这是 PowerShell 5.1 的已知设计缺陷，不是 bug 可以被修复。

### Windows 命令行参数编码（all modules）

> **此问题影响所有通过 `node ima_api.cjs` 的调用（notes、knowledge-base 的检索/写入均受影响），与 PowerShell 版本无关。**
>
> **Windows 上 agent 执行的命令会经过临时脚本/代码页转换链路：命令字符串中的中文以 UTF-8 字节写入临时脚本，被 PowerShell 5.1（或 cmd 的 ANSI 代码页）按 GBK 解码后，中文已变成乱码，再传给 node 作为命令行参数。结果是 `{"query":"高血压"}` 中的关键词到达 IMA 时已是乱码，检索结果必然错误。**

**规则（强制）：body 中含有任何中文/非 ASCII 字符时，禁止把 body 直接内联在命令行参数中。** 必须使用以下两种方式之一：

1. **body 文件（推荐）：** 用 `write_to_file`（UTF-8，无 BOM）或 `Set-Content -Encoding UTF8` 将完整 body JSON 写入临时文件，调用时传 `@绝对路径`。脚本自动按 UTF-8 读取并剥离 BOM。
   ```powershell
   # ① 先写 body 文件（write_to_file 写出的是 UTF-8）
   #    {"query": "高血压治疗指南", "knowledge_base_id": "...", "cursor": ""}
   # ② 再调用（body 参数为 @文件路径，不含任何中文）
   $resp = node "$SKILL_DIR/ima_api.cjs" "openapi/wiki/v1/search_knowledge" "@C:\Users\...\AppData\Local\Temp\search_body.json" "$OPTS"
   $data = $resp | ConvertFrom-Json
   ```
2. **stdin 管道（仅纯 ASCII 内容）：** body 参数传 `-`，内容经管道传入。**中文经 Windows 管道同样会被破坏（已实测），含中文时不要用此方式。**
   ```powershell
   $body = '{"limit": 10}'
   $resp = $body | node "$SKILL_DIR/ima_api.cjs" "openapi/list_docs" "-" "$OPTS"
   ```

> **判断依据：** 调用 `node ima_api.cjs "openapi/wiki/v1/search_knowledge" '{"query":"高血压"}' "$OPTS"` 后，如果返回 `code=220004`（invalid knowledge_base_id）之类的异常错误、或结果明显与关键词不符，通常就是命令行中文被 GBK 误读导致 —— 改用 body 文件（`@路径`）即可。

### 检索结果乱码排查清单

当 agent 报告检索（`search_knowledge` / `search_note`）或浏览（`get_media_info` / `get_doc_content`）返回的内容乱码时，按以下顺序排查：

1. **命令行参数是否直接包含中文 query？** → 是，这就是根因。改用 body 文件（`@路径`）或 stdin（`-`），见 [Windows 命令行参数编码](#windows-命令行参数编码all-modules)。
2. **PowerShell 5.1 下是否设置了 `[Console]::OutputEncoding`？** → 未设置则捕获 stdout 时 UTF-8 被按 GBK 解码。任何 `node ima_api.cjs` 调用前先设置。
3. **知识库原文在 IMA 客户端（https://ima.qq.com）中打开是否乱码？** → 是，说明是**历史脏数据**（早期用 `Invoke-RestMethod` 直接写入时已乱码），**无法通过代码修复**，只能删除后在 IMA 客户端/本 skill 重新上传。
4. **是否误用了 `res.text()` 依赖响应头 charset？** → 本 skill 的 `ima_api.cjs` 已改为显式 UTF-8 解码（带 GBK 回退），无需处理；若自行调用 IMA API，请按同样的方式解码。
5. **后端（Python `ima_client.py` 等）检索乱码？** → 检查 `_bytes_to_text` 等解码函数是否"UTF-8 优先"导致 GBK 内容被误判为合法 UTF-8（字节恰好合法但语义乱码）。应按响应头 charset 解码或先试 GBK/GB18030。
