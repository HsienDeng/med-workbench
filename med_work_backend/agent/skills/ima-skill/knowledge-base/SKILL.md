...
> **⚠️ Windows 环境检索乱码重点：** `query` 是中文时，**禁止**将内联 body 直接写在命令行参数里（Windows 的命令行字节会经过 GBK 转换链路，中文到达脚本时已乱码），stdin 管道对中文也不可靠。必须把 body 写入 UTF-8 文件后传 `@路径`：
>
> ```bash
> # ① 先用 write_to_file 写入 UTF-8 body 文件，例如 search_body.json：
> #    {"query": "高血压治疗指南", "knowledge_base_id": "<kb_id>", "cursor": ""}
> # ② 再调用（body 参数为 @文件路径，不含任何中文）
> ima_api "openapi/wiki/v1/search_knowledge" "@/tmp/search_body.json"
> ```
