...
> **⚠️ Windows 环境检索乱码重点：** 搜索关键词（`title` / `content`）是中文时，**禁止**把内联 body 直接写在命令行参数里（Windows 的命令行字节会经过 GBK 转换链路，中文到达脚本时已乱码），stdin 管道对中文也不可靠。必须把 body 写入 UTF-8 文件后传 `@路径`：
>
> ```bash
> # ① 先用 write_to_file 写入 UTF-8 body 文件，例如 search_note_body.json：
> #    {"search_type": 0, "query_info": {"title": "会议纪要"}, "start": 0, "end": 20}
> # ② 再调用（body 参数为 @文件路径，不含任何中文）
> ima_api "openapi/note/v1/search_note" "@/tmp/search_note_body.json"
> ```
> 详见根目录 `SKILL.md` 的「Windows 命令行参数编码（all modules）」和「检索结果乱码排查清单」。
